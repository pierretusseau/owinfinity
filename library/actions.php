...
////// Gallerie de héro HOMEPAGE ////////////////////////////////////////////////////////////////
	function HomeLoopBlog($lang) {
		...
		// Si je n'ai pas assez d'articles
		$postCount = wp_count_posts();
		$postCounting = ceil(($postCount->publish) / 2);
		if ($postCounting < 6) :
			for ($i=$postCounting; $i < 6 ; $i++) {
				require(TEMPLATEPATH.'/template-parts/loop-homev2-blog-empty.php');
			}
		endif;
		// Je reste mes requêtes de post
		wp_reset_postdata();
	}
	add_action('HomeLoopBlog','HomeLoopBlog',2,1);
	// 	NOM DE L'ACTION / FUNCTION / PRIORITE / NOMBRE DE PARAMETRE
