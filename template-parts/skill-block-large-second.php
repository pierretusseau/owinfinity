<?php
	$needLine2 = 0;
	if (!($skill == 'ult')) :
		if (!empty(get_field($skill.'2_cooldown'))) :
		$skillCooldown = get_field($skill.'2_cooldown');
		$needLine2++;
		endif;
	endif;
	if (!empty(get_field($skill.'2_range'))) :
		$skillRange = get_field($skill.'2_range');
		$needLine2++;
	endif;
	if (!empty(get_field($skill.'2_damage'))) :
		$skillDamage = get_field($skill.'2_damage');
		$needLine2++;
	endif;
	if (!empty(get_field($skill.'2_duration'))) :
		$skillDuration = get_field($skill.'2_duration');
		$needLine2++;
	endif;
?>
<header class="row">
